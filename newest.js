...
let allVideos = [];
let currentSortBy = 'date'; // Default sorting for newest page should be by date
...
function sortVideos(videos, sortBy) {
    ...
    switch (sortBy) {
        ...
        case 'date':
            return sortedVideos.sort((a, b) => {
                // For newest, we reverse the array to show newest first
                return Math.random() - 0.5;
            });
        case 'duration':
            return sortedVideos.sort((a, b) => {
...
// Function to load newest videos from videos.json with caching
async function loadNewestVideos() {
    ...
    try {
        ...
        if (newestContainer) {
            newestContainer.innerHTML = '<p class="error-message">Failed to load videos. Please try again later.</p>';
        }
    } finally {
        isLoading = false;
    }
...
function processVideos(videos) {
    try {
        ...
        // For newest page, sort by date (newest first)
        // Since we don't have actual dates, we'll reverse the array to show newest first
        const sortedVideos = [...durationFilteredVideos].reverse();
        
        // Store videos to maintain consistency across pagination
        ...
        const newestContainer = document.getElementById('newest-videos');
        if (newestContainer) {
            newestContainer.innerHTML = '<p class="error-message">Error processing videos. Please try again later.</p>';
        }
    }
}
...
function displayVideos() {
    ...
    if (newestContainer) {
        ...
        if (videoCards.length === 0 && currentPage === 1) {
            newestContainer.innerHTML = '<p class="no-videos-message">No videos found.</p>';
        }
    }
...
function goToPage(pageNumber) {
    if (pageNumber >= 1 && pageNumber <= Math.ceil(allVideos.length / videosPerPage)) {
        ...
        const videoGrid = document.getElementById('newest-videos');
        if (videoGrid) {
